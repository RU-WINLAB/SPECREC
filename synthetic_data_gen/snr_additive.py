...
class snr_additive(gr.top_block):

    def __init__(self, filename='file1.bin', input_source_select=1, modulation='bpsk', noise_seed=1, num_samps=1000, samp_rate=20e6, snr=30, sourcefile='source.bin'):
        gr.top_block.__init__(self, "SNR With Additive Noise")

        ##################################################
        # Parameters
        ##################################################
        self.filename = filename
        self.input_source_select = input_source_select
        ...
        # Variables
        ##################################################
        self.signal_gain = signal_gain = 10 ** (snr / 20) * 0.5 if modulation == "qpsk" else 10 ** ((snr+6) / 20) * 0.5
    ...
    def get_input_source_select(self):
        return self.input_source_select

    def set_input_source_select(self, input_source_select):
        self.input_source_select = input_source_select
    ...
    def set_modulation(self, modulation):
        ...
        self.set_signal_gain(10 ** (self.snr / 20) * 0.5 if self.modulation == "qpsk" else 10 ** ((self.snr+6) / 20) * 0.5)
...
def argument_parser():
    ...
    parser.add_argument(
        "--input-source-select", dest="input_source_select", type=intx, default=1,
        help="Set input source select [default=%(default)r]")
    parser.add_argument(
        "--modulation", dest="modulation", type=str, default='bpsk',
        help="Set Modulation type ('bpsk', 'cpm', 'gmsk', 'qpsk', '16qam', 8psk') [default=%(default)r]")
    parser.add_argument(
        "--noise-seed", dest="noise_seed", type=intx, default=1,
        help="Set noise seed [default=%(default)r]")
    parser.add_argument(
        "--num-samps", dest="num_samps", type=intx, default=1000,
    ...
    parser.add_argument(
        "--snr", dest="snr", type=eng_float, default="30.0",
...
def main(top_block_cls=snr_additive, options=None):
    if options is None:
        options = argument_parser().parse_args()
    tb = top_block_cls(filename=options.filename, input_source_select=options.input_source_select, modulation=options.modulation, noise_seed=options.noise_seed, num_samps=options.num_samps, samp_rate=options.samp_rate, snr=options.snr, sourcefile=options.sourcefile)
