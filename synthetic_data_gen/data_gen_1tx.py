...
class data_gen_1tx(gr.top_block):

    def __init__(self, filename='file1.bin', gain=1.0, modulation='bpsk', noise_seed=0, num_samps=1000000, samp_rate=20e6, sig1_bw=4e6, sig1_freq=7e6, snr=10):
        gr.top_block.__init__(self, "Generic Single Transmitter Data Gen")
...
def main(top_block_cls=data_gen_1tx, options=None):
    if options is None:
        options = argument_parser().parse_args()
    tb = top_block_cls(filename=options.filename, gain=options.gain, modulation=options.modulation, noise_seed=options.noise_seed, num_samps=options.num_samps, samp_rate=options.samp_rate, sig1_bw=options.sig1_bw, sig1_freq=options.sig1_freq, snr=options.snr)
