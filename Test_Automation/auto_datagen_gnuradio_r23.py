...
def argument_parser():
    ...
    parser.add_argument(
        ...
        help="Number of samples to collect [default=%(default)r]")
    parser.add_argument(
        "--gain-scale", dest="gain_scale", type=float, 
        default= 1.0,
        help="Value to scale the output gain by [default=%(default)r]")
    return parser
#%% General functions
def genDatecode():
...
def genInfoAdditiveSNR(mod1, snr, gain_sig = 1, gain_noise = 0, gain_scale = 1):
    return (" --modulation " + mod1 + " --snr " + str(snr) +  " --gain-scale " + str(gain_scale) + " ")
        ...
        for mod2 in modulation2:
            if mod1 != mod2:    
                for i in range(1, int(num_iter) + 2):
                    ...
                    if glVar.script_file.find("snr") > -1: glVar.info_full =("python3 " + glVar.script_file + genInfoAdditiveSNR(mod1, snr, gain_sig = gain_sig, gain_noise = gain_noise, gain_scale = gain_scale))
