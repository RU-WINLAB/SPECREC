...
from keras.layers import Dense, Flatten, Input, Dropout, UpSampling2D
...
class NN():
    ...
    def runNN(self, X_train, Y_train, X_test, Y_test, X_val, Y_val, 
              ...
              epochs = 10, batch_size = 128, tesrtAct = False, mod = '',
              train_model = True, folder_NN_hist = "NN_Hist", testAct = False):

        if train_model: print("The training model is set to ")
        ...
        weight_file = os.path.join(folder_NN_hist, "CAT_weights.h5"); 
        model_file = os.path.join(folder_NN_hist, "CAT_model"); 
        hist_file = os.path.join(folder_NN_hist, "CAT_history.csv");
...
def test():
    ...
    NNet.runNN(X_train, Y_train, X_test, Y_test,
                                  X_val, Y_val,epochs=10, train_model= True)
    #print(pred)
